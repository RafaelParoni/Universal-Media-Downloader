...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.service_cards = []
        # Replicating the bug exactly: all cards initialized at row=1, col=0 inside loop
        for i in range(14):
        ...
        self.current_cols = 0
        self.card.bind("<Configure>", self.reorganize_cards)
        
    def reorganize_cards(self, event):
        if event.widget == self.card:
            available_width = event.width
            cols = max(1, available_width // 180) 
            print(f"Reorganize called: width={available_width}, cols={cols}")
...
if __name__ == "__main__":
    app = App()
    app.after(500, lambda: app.geometry("900x600"))
    app.after(1000, lambda: app.geometry("400x600"))
    app.after(1500, lambda: app.geometry("500x600"))
